...
def plot_tims_profile_heatmap(
    ...
    max_scan, min_scan = tims_data.scan_max_index - np.searchsorted(
        tims_data.mobility_values[::-1], [im_slice.start, im_slice.stop], side="right"
    )
    ...
    return px.imshow(np.stack(img_datas, axis=0), facet_col=0, facet_col_wrap=n_cols)
...
def _make_dense_numba(
    ...
    for frame, scan, inten in zip(frame_indices, scan_indices, intensity_values):
