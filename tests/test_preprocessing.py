...
def test_get_mq_unique_proteins():
    ...
    assert len(proteins) == 6, "The number of extracted proteins is wrong."
    assert (
        "Plectin" in proteins
...
def test_get_identified_ions():
    ...
    assert [False, True, True, False, True, False] == preproc.get_identified_ions(
        all_ions, peptide, "b"
    ), "The extraction of identified b-ions doesn't work"
    assert [False, False, True, False, False, True] == preproc.get_identified_ions(
        all_ions, peptide, "y"
...
def test_convert_diann_mq_mod():
    ...
    assert "VSHGSSPSLLEALSSDFLAC[Carbamidomethyl (C)]K" == preproc.convert_diann_mq_mod(
        seq1
    )
    seq2 = "VSVINTVDTSHEDMIHDAQM(UniMod:35)DYYGTR"
    assert "VSVINTVDTSHEDMIHDAQM[Oxidation (M)]DYYGTR" == preproc.convert_diann_mq_mod(
        seq2
    )
    ...
    assert (
        "[Acetyl (N-term)]VSHGSSPSLLEALSSDFLAC[Carbamidomethyl (C)]K"
        == preproc.convert_diann_mq_mod(seq1_several_dif_mods)
    )
    seq2_several_same_mods = "CAALVATAEENLC(UniMod:4)C(UniMod:4)EELSSK"
    assert (
        "CAALVATAEENLC[Carbamidomethyl (C)]C[Carbamidomethyl (C)]EELSSK"
        == preproc.convert_diann_mq_mod(seq2_several_same_mods)
    )
...
def test_convert_diann_ap_mod():
    ...
    assert (
        preproc.convert_diann_ap_mod(seq1) == "pgELNMIIMLPDETTDLR"
    ), "The convertion of the N-term modifications doesn't work."

    seq2 = "ADFSGM(UniMod:35)SQTDLSLSK"
    assert (
        preproc.convert_diann_ap_mod(seq2) == "ADFSGoxMSQTDLSLSK"
    ), "The convertion of the modifications doesn't work."

    seq3 = "YYYDGDMIC(UniMod:4)"
    assert (
        preproc.convert_diann_ap_mod(seq3) == "YYYDGDMIcC"
    ), "The convertion of the C-term modifications doesn't work."

    seq1_several_dif_mods = "(UniMod:1)VSHGSSPSLLEALSSDFLAC(UniMod:4)K"
    assert "aVSHGSSPSLLEALSSDFLAcCK" == preproc.convert_diann_ap_mod(
        seq1_several_dif_mods
    )
    seq2_several_same_mods = "CAALVATAEENLC(UniMod:4)C(UniMod:4)EELSSK"
    assert "CAALVATAEENLcCcCEELSSK" == preproc.convert_diann_ap_mod(
        seq2_several_same_mods
    )
    seq_no_mod = "CVNTTLQIK"
