...
PACKAGE_NAME=alphaviz
# BUILD_NAME is taken from environment variables, e.g. alphaviz-1.3.0-macos-darwin-arm64 or alphaviz-1.3.0-macos-darwin-x64
...
mv dist_pyinstaller/alphaviz_gui ${CONTENTS_FOLDER}/MacOS
cp release/macos/Info.plist ${CONTENTS_FOLDER}
cp release/macos/alphaviz_terminal ${CONTENTS_FOLDER}/MacOS
...
pkgbuild --root dist_pyinstaller/${PACKAGE_NAME} --identifier de.mpg.biochem.${PACKAGE_NAME}.app --version 1.3.1-dev0 --install-location /Applications/${PACKAGE_NAME}.app --scripts release/macos/scripts ${PACKAGE_NAME}.pkg
